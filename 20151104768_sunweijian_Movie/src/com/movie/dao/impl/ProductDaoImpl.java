...
import com.movie.utils.DataSourceUtils;
...
public class ProductDaoImpl implements ProductDao {
	...
	public List<Product> findByPage(PageBean<Product> pb, String cid) throws Exception {
		QueryRunner qr = new QueryRunner(DataSourceUtils.getDataSource());
		String sql = "select * from product where cid = ? & pflag = ? order by pdate desc limit ?,?";
	...
	public void save(Product p) throws Exception {
		 ...
		 */
		String sql="insert into product(`pid`, `pname`, `market_price`, `shop_price`,"
				+ "`pimage`, `is_hot`, `pdesc`, `pflag`, `cid`) values(?,?,?,?,?,?,?,?,?);";
		qr.update(sql, p.getPid(),p.getPname(),p.getMarket_price(),
				p.getShop_price(),p.getPimage(),
